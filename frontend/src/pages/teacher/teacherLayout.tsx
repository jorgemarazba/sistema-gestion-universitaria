...
import { Bell, User, LogOut, LayoutDashboard, BookOpen, Users, Award, Clock, FolderOpen, MessageSquare, BarChart3 } from 'lucide-react';
...
const navItems: NavItem[] = [
  ...
  { label: 'Mis Cursos', path: '/teacher/cursos', icon: <BookOpen size={18} /> },
  { label: 'Estudiantes', path: '/teacher/estudiantes', icon: <Users size={18} /> },
  { label: 'Calificaciones', path: '/teacher/calificaciones', icon: <Award size={18} /> },
  { label: 'Horario', path: '/teacher/horario', icon: <Clock size={18} /> },
  { label: 'Materiales', path: '/teacher/materiales', icon: <FolderOpen size={18} /> },
  { label: 'Mensajes', path: '/teacher/mensajes', icon: <MessageSquare size={18} /> },
  { label: 'Reportes', path: '/teacher/reportes', icon: <BarChart3 size={18} /> },
];
...
export const TeacherLayout: React.FC = () => {
  ...
  const notificaciones = [
    { id: 1, titulo: 'Nuevo estudiante', descripcion: 'Un estudiante se inscribió en tu curso', tiempo: '1 h', leida: false },
    { id: 2, titulo: 'Tarea entregada', descripcion: 'María García entregó la tarea de Matemáticas', tiempo: '3 h', leida: true },
    { id: 3, titulo: 'Mensaje de coordinación', descripcion: 'Reunión de profesores el lunes', tiempo: '1 d', leida: true },
  ];
    ...
    <div className="flex flex-col h-screen font-sans" style={backgroundStyle}>
      <header className="h-16 bg-slate-900/95 border-b border-slate-700 flex items-center justify-between px-4 sticky top-0 z-50 shadow-lg backdrop-blur-sm">
        <div className="flex items-center gap-3">
          <div className="h-10 w-10 bg-linear-to-br from-emerald-500 to-teal-600 rounded-xl flex items-center justify-center shadow-lg">
            <span className="text-white font-bold text-lg">P</span>
          </div>
          <span className="text-white font-bold text-xl hidden lg:block">Profesor</span>
        </div>
        ...
        <div className="flex items-center gap-3">
            ...
            {notifOpen && (
              <div className="absolute right-0 top-full mt-2 w-80 bg-slate-800 rounded-xl shadow-2xl border border-slate-700 z-50 max-h-[400px] overflow-y-auto">
                  ...
                  {notificaciones.map((notif) => (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-emerald-400 text-xs mt-1">{notif.tiempo}</p>
                      </div>
                      {!notif.leida && (
                ...
                <div className="p-3 border-t border-slate-700 text-center">
                  <button className="text-emerald-400 text-sm hover:text-emerald-300 font-medium">
                    Ver todas
                  </button>
                </div>
              </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="text-white text-sm font-medium hidden lg:block">Profesor</span>
          </div>
